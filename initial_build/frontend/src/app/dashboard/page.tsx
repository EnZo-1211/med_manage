      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="p-4 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
            <select className="bg-white border border-gray-200 text-gray-700 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block p-2 w-full sm:w-auto">
              <option>All Patients</option>
              <option>Active</option>
              <option>Inactive</option>
            </select>
            
            <button onClick={() => router.push('/dashboard/add')} className="bg-[#FF6600] hover:bg-[#E65C00] text-white px-4 py-2 rounded-lg text-sm font-medium flex justify-center items-center transition-colors shadow-sm whitespace-nowrap w-full sm:w-auto">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50 border-b border-gray-200">
                ...
                <th className="hidden md:table-cell px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Age</th>
                <th className="hidden md:table-cell px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Added By</th>
                <th className="hidden md:table-cell px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Last Updated</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {patients.map((patient) => (
                <tr key={patient.id} className="hover:bg-gray-50/50 transition-colors group cursor-pointer" onClick={() => router.push(`/dashboard/patients/${patient.id}`)}>
                  ...
                  <td className="hidden md:table-cell px-6 py-4 text-sm text-gray-600">{calculateAge(patient.date_of_birth)}</td>
                  <td className="hidden md:table-cell px-6 py-4 text-sm text-gray-600">{patient.added_by_name || "Unknown"}</td>
                  <td className="hidden md:table-cell px-6 py-4 text-sm text-gray-500">{timeAgo(patient.last_updated_at || patient.updated_at || patient.created_at)}</td>
